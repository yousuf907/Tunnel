...
from datasets_no_aug import build_dataset, filter_by_class ## without augmentation
from engine_std import train_one_epoch, evaluate, visualize
...
def get_args_parser():
    ...
    parser.set_defaults(repeated_aug=False) # No aug
    ...
    parser.add_argument('--mixup', type=float, default=0,
                        help='mixup alpha, mixup enabled if > 0. (default: 0)') # No aug
    parser.add_argument('--cutmix', type=float, default=0,
                        help='cutmix alpha, cutmix enabled if > 0. (default: 0)') # No aug
    ...
    parser.add_argument('--ckpt_name', type=str, default='checkpoint.pth')
...
def main(args):
    ...
    if mixup_active:
        print("Using any Mixup/Cutmix")
        mixup_fn = Mixup(
            mixup_alpha=args.mixup, cutmix_alpha=args.cutmix, cutmix_minmax=args.cutmix_minmax,
            prob=args.mixup_prob, switch_prob=args.mixup_switch_prob, mode=args.mixup_mode,
            label_smoothing=args.smoothing, num_classes=args.nb_classes)
    else:
        print("Not using any Mixup/Cutmix")
    ...
    if args.finetune:
        ...
        #for param_tensor in checkpoint_model:
        #    print(param_tensor, "\t", checkpoint_model[param_tensor].size())
    ...
    if args.mixup > 0.:
        ...
        print("Using label smoothing of ", args.smoothing)
    ...
    if args.eval:
        ...
        print(f"Accuracy of the network on the {len(val_idx)} test images: {test_stats['acc1']:.1f}%")
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            #checkpoint_paths = [output_dir / 'checkpoint_32.pth']
            checkpoint_paths = [output_dir / args.ckpt_name]
        ...
        is_best = test_stats["acc1"] > max_accuracy
        max_accuracy = max(max_accuracy, test_stats["acc1"])
        print(f'Max accuracy: {max_accuracy:.2f}%')
        
        if is_best:
            shutil.copyfile(os.path.join(output_dir, args.ckpt_name), os.path.join(output_dir, 'best_' + args.ckpt_name))            
