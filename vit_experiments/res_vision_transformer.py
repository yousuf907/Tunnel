...
# --------------------------------------------------------
# The following code is based on timm library
# --------------------------------------------------------
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        #x = self.drop_path(self.mlp(self.norm2(x))) ## skip connections omitted
        if 0:
            x.data[:,0,0] *= 0.
            x.data[:,0,0] -= 3.
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        #self.pos_embed = torch.zeros(1, num_patches + self.num_tokens, embed_dim).cuda()
        self.pos_drop = nn.Dropout(p=drop_rate)

        self.pos_embed = posemb_sincos_2d(
    ...
    def get_intermediate_layers(
        ...
        #if norm:
        outputs = [self.norm(out) for out in outputs] ## helps so keep it!
    ...
    def dynamic_forward(self, x, lst=None):
        # x = self.forward_features(x)
        ...
        bs, token_cnt, emb = x.shape

        # x = self.blocks(x)
        for i, block in enumerate(self.blocks):
            # print(token_cnt)
            # x = x[:,:int(token_cnt*0.8),:]
            if lst is not None:
                x = x[:,:int(token_cnt*lst[i]),:]
                # print(int(token_cnt*lst[i]))
            x = block(x)
