...
class ResNet(nn.Module):
    ...
    def __init__(self, block: BasicBlock, num_blocks: List[int],
        ...
        self.nf = nf
        self.conv1 = conv3x3(3, nf * 1) # stride=1 for resolution 32x32 & 64x64
        #self.conv1 = conv3x3(3, nf * 1, stride=2) # stride=2 for res 128x128 & 224x224
        self.bn1 = nn.BatchNorm2d(nf * 1)
        #self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1) # only for res 128x128 & 224x224
        self.layer1 = self._make_layer(block, nf * 1, num_blocks[0], stride=1) # stride=1 for all res > DO NOT CHANGE
    ...
    def forward(self, x: torch.Tensor, returnt='out') -> torch.Tensor:
        ...
        #print('Conv-4:', out.shape)
        out = self.layer4(out)  # -> 512, 4, 4
        #print('Conv-5:', out.shape)
        out = avg_pool2d(out, out.shape[2]) # -> 512, 1, 1
        #print('Avg Pooled:', out.shape)
        feature = out.view(out.size(0), -1)  # 512
        #print('Flattened:', feature.shape)
        if returnt == 'features':
            return feature
